...
function Manager::ManageLoan()
{
	...
	Util.Debug("globman", 0, "Managing loan");
	if (AICompany.GetBankBalance(AICompany.COMPANY_SELF) >= MONEY_BEFORE_LOAN_REDUCE) {
		Util.Debug("globman", 0, "The company now has more than £" + MONEY_BEFORE_LOAN_REDUCE + ". Reducing Loan to minimum");
...
function Manager::ManageEvents()
{
	...
	if (AIEventController.IsEventWaiting()) {
		Util.Debug("globman", 0, "Managing events");
	}
	while (AIEventController.IsEventWaiting()) {
		...
		switch (event.GetEventType()) {
			case AIEvent.AI_ET_COMPANY_BANKRUPT:
				...
				if (!companyname) Util.Debug("globman", 0, "A company has gone bankrupt. Better luck next time..");
				else Util.Debug("globman", 0, companyname + " has gone bankrupt. Better luck next time..");
				break;
			case AIEvent.AI_ET_COMPANY_IN_TROUBLE:
				...
				Util.Debug("globman", 0, companyname + " is in trouble and may go bankrupt soon");
				break;
			case AIEvent.AI_ET_COMPANY_MERGER:
				...
				Util.Debug("globman", 0, oldcompanyname + " has been bought by " + newcompanyname);
				break;
			case AIEvent.AI_ET_COMPANY_NEW:
				local companyname = AICompany.GetName(AIEventCompanyNew.Convert(event).GetCompanyID());
				Util.Debug("globman", 0, companyname + " has just started");
				break;
			...
			default:
				Util.Debug("globman", 1, GetEventName(event.GetEventType()) + " is unhandled");
				break;
		}
	}
	if (dealtWithEvent) {
		Util.Debug("globman", 0, "No more events to manage");
	}
}
